...
HRESULT CD3DFont::Print( QString text, SRColor color, float x, float y, bool skipColorTags, bool noColorFormat )
{
	...
	text.replace("\t", "\x20\x20\x20\x20");
	float xp = x, yp = y;
	D3DCOLOR clr = color.argb;
	...
	for ( size_t cpos = 0; cpos < len; cpos++ )
	{
		...
		int c = ( byte ) ( text.toLocal8Bit()[cpos] ) - 32;
		if ( !( c >= 0 && c < 224 ) )
			continue;
...
HRESULT CD3DFont::PrintShadow (float x, float y, SRColor color, QString text )
{
	...
	if (color.red >= 0x80 && color.green >= 0x80 && color.blue >= 0x80)
		Print(text, SRColor(eCdBlack, color.alpha), x + 1, y + 1, true, false);
	else Print(text, SRColor(eCdWhite, color.alpha), x + 1, y + 1, true, false);
	//}
	return Print(text, color, x, y, false, false);
}
...
HRESULT CD3DRender::D3DColor (SRColor color )
{
	m_color = color.argb;
...
void CD3DRender::D3DBox (float x, float y, float w, float h, SRColor color )
{
	if ( SUCCEEDED(Begin(D3DPT_TRIANGLELIST)) )
	{
		D3DColor( color );
...
void CD3DRender::D3DBoxi (int x, int y, int w, int h, SRColor color, int maxW )
{
	if ( maxW )
	{
		if ( w >= maxW )
...
void CD3DRender::D3DBoxBorder (float x, float y, float w, float h, SRColor border_color, SRColor color )
{
...
void CD3DRender::D3DBoxBorderi (int x, int y, int w, int h, SRColor border_color, SRColor color )
{
	D3DBoxBorder( (float)x, (float)y, (float)w, (float)h, border_color, color );
}

void CD3DRender::D3DLine(int x, int y, int x2, int y2, SRColor color)
{
...
bool CD3DRender::DrawLine (const D3DXVECTOR3 &a, const D3DXVECTOR3 &b, SRColor color )
{
	if ( FAILED(CD3DBaseRender::BeginRender()) )
		return false;
	...
	m_pD3Dbuf->Lock( 0, 0, (void **) &lineList, D3DLOCK_DISCARD | D3DLOCK_NOSYSLOCK ); // flogs: D3DLOCK_NOSYSLOCK, D3DLOCK_DISCARD
	{
		...
		lineList[0].y = a.y;
		lineList[0].z = a.z;
		lineList[0].color = color.argb;
		lineList[0].specular = color.argb;

		lineList[1].x = b.x;
		lineList[1].y = b.y;
		lineList[1].z = b.z;
		lineList[1].color = color.argb;
		lineList[1].specular = color.argb;
	}
